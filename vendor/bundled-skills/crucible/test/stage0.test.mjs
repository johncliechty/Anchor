...
test('correct tier selection by input (greenfield / T1 docs-only / T2 repo / T3 large|contested)', () => {
  ...
  assert.equal(selectTier({ repoDir: '<path>' }).tier, TIERS.TIER2, 'a repo ⇒ Tier 2');
  assert.equal(selectTier({ repoDir: '<path>', large: true }).tier, TIERS.TIER3, 'large ⇒ Tier 3');
...
test('Tier 2 reproduce-first downgrades a Confirmed-but-untested claim to Inferred', async () => {
  ...
  const manifest = await runIngest({ tier: TIERS.TIER2, input: { repoDir: '<path>' }, agent });
...
test('Tier 3 delegates deep archaeology to the researchPrime lane and folds its findings in', async () => {
  ...
  const manifest = await runIngest({ tier: TIERS.TIER3, input: { repoDir: '<path>', large: true }, agent, research });

  assert.equal(manifest.archaeologyRun, true);
  assert.deepEqual(research.digs, ['<path>'], 'archaeology ran for the project dir');
...
test('Tier 3 without a research coordinator HALTs (Crucible never re-implements the pipeline)', async () => {
  ...
  await assert.rejects(
    () => runIngest({ tier: TIERS.TIER3, input: { repoDir: '<path>', large: true }, agent, research: null }),
...
test('Given a criterion-touching Gap, then Stage 0 HALTs rather than locking', async () => {
  ...
  await assert.rejects(
    () => runStage0({ intent: INTENT, input: { repoDir: '<path>' }, agent, intakeDir: dir, salvageAnswer: 'salvage', approved: true }),
