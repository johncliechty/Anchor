...
test('Tier-3 deep-archaeology delegates to researchPrime and is cost-guarded per dir', async () => {
  ...
  const dig = await research.deepArchaeology({ projectDir: '<path>' });
  ...
  assert.equal(runResearch.specs[0].projectDir, '<path>');

  // Re-entering Stage 0 on the same tree does NOT re-excavate.
  const dig2 = await research.deepArchaeology({ projectDir: '<path>' });
