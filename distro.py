...
_EMAIL_ALLOWLIST = frozenset({
    ...
    "<email>",   # Claude Code co-author trailer
})
...
def select_shippable(root: Path | None = None,
    ...
    for rel_posix, _abs in _iter_repo_files(root):
        if _match_any(rel_posix, patterns):
...
# ``<path><seg>`` / ``<path><seg>`` home path leaks the build host's
...
# POSIX home paths: <path><seg> and <path><seg>. Requires a concrete
...
# canonical build root (``<path> is the project's own folder, used
...
# NOTE on the trio/skill-source leaves: ``<path> / ``<path>
...
_HOSTPATH_ALLOWED_LEAVES = frozenset({
    "anchor",     # the project's own root (<path> — REPO_ROOT.name
